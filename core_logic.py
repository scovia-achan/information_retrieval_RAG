...
# create embeddings
def create_and_store_embeddings(chunks, vectorstore_path="vector_store"):
...
def load_retriever(vectorstore_path="vector_store"):
    """Loads the saved FAISS index and returns it as a LangChain retriever."""
    embeddings = HuggingFaceEmbeddings("sentence-transformers/all-MiniLM-L6-v2")
    vectore_store = FAISS.load_local(vectorstore_path, embeddings, allow_dangerous_deserialization=True)
